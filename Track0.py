...
start_time = datetime(2015,2,18,16,39,0,0,pytz.UTC)#datetime.now(pytz.UTC)-timedelta(model_days) 
...
if Option==2: # user specified pts
    point1 = (41.91875000000001,-70.329637096774192)  # Point data structure:(lat,lon)42.0358930225,-70.1035802334,41.912806, -70.152132
...
if Option == 1:
    ...
    dr_points = extract_drifter_point(drifter_ID,INPUT_DATA,start_time,model_days)
    ...
    if MODEL=='FVCOM':
        point1 = extract_fvcom_point(dr_points['time'][0],dr_points['time'][-1],dr_points['lon'][0],dr_points['lat'][0],GRID,track_way)
        end_time2 = dr_points['time'][-1] + timedelta(model_days)
        point2 = extract_fvcom_point(dr_points['time'][-1],end_time2,dr_points['lon'][-1],dr_points['lat'][-1],GRID,track_way)
    if MODEL=='ROMS':
        point1 = extract_roms_point(dr_points['time'][0],dr_points['time'][-1],dr_points['lon'][0],dr_points['lat'][0],track_way)
        end_time2 = dr_points['time'][-1] + timedelta(model_days)
        point2 = extract_roms_point(dr_points['time'][-1],end_time2,dr_points['lon'][-1],dr_points['lat'][-1],track_way)
...
if Option==2 or Option==3:
    ...
    for i in range(stp_num):
        if MODEL=='FVCOM': point = extract_fvcom_point(start_time,end_time,st_lon[i],st_lat[i],GRID,track_way)        
        if MODEL=='ROMS': point = extract_roms_point(start_time,end_time,st_lon[i],st_lat[i],track_way)
...
if Option == 1:
    ...
    if image_style=='plot':
        ax.annotate(an1,xy=(dr_points['lon'][0],dr_points['lat'][0]),xytext=(dr_points['lon'][0]+0.01*model_days,dr_points['lat'][0]+0.01*model_days),
                    fontsize=6,arrowprops=dict(arrowstyle="wedge")) #facecolor=colors[i]''
        ax.annotate(an2,xy=(dr_points['lon'][-1],dr_points['lat'][-1]),xytext=(dr_points['lon'][-1]+0.01*model_days,dr_points['lat'][-1]+0.01*model_days),
                    fontsize=6,arrowprops=dict(arrowstyle="wedge")) #facecolor=colors[i]'''
...
if Option==2 or Option==3:    
    ...
    if image_style=='plot':
        for j in range(stp_num):
            ax.annotate('Start %d'%(j+1), xy=(lon_set[j][0],lat_set[j][0]),xytext=(lon_set[j][0]+0.01*model_days,lat_set[j][0]+0.01*model_days),
                        fontsize=6,arrowprops=dict(arrowstyle="wedge")) #facecolor=colors[i]'''
