...
class get_fvcom(track):
    ...
    def nearest_point_index(self, lon, lat, lons, lats):  #,num=4
        ...
        #print 'index',index
        return index,lonp,latp,distance   #index,
    def get_url(self, starttime, endtime):
        '''
        ...
        if self.modelname is "30yr":
            ...
            lon[0:1:98431],lat[0:1:98431],lonc[0:1:165094],latc[0:1:165094],siglay[0:1:9][0:1:98431],h[0:1:98431],u[{0}:1:{1}][0:1:9][0:1:165094],
            v[{0}:1:{1}][0:1:9][0:1:165094],nv[0:1:2][0:1:165094],nbe[0:1:2][0:1:165094]'''
            current_time = pytz.utc.localize(datetime.now().replace(hour=0,minute=0))
            period = starttime-(current_time-timedelta(days=3))
            if period.total_seconds()<0:
    ...
    def __get_track(self, lon, lat, url,track_way): #depth,
        ...
        nodes = dict(lon=[lon], lat=[lat])
        try:
            kf,lona,lata,distanceF = self.nearest_point_index(lon,lat,lonc,latc)#,num=1kf,
        ...
        for i in range(t):
            ...
            nodes['lon'].append(lon);  nodes['lat'].append(lat)
            try:
                kf,lonz,latz,distanceF = self.nearest_point_index(lon, lat, lonc, latc)
